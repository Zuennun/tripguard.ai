export type Locale = "de" | "en";

export const translations = {
  de: {
    // Brand
    brand: "SaveMyHoliday",
    brandTagline: "SaveMyHoliday.",
    brandPart1: "Save",
    brandPart2: "My",
    brandPart3: "Holiday",

    // Navbar
    nav: {
      home: "Start",
      howItWorks: "So funktioniert's",
      trackHotel: "Hotel tracken",
      priceGuide: "Preis-Guide",
      cities: "Städte",
      faq: "FAQ",
      contact: "Kontakt",
      blog: "Blog",
      cta: "Kostenlos Preisüberwachen",
      ctaMobile: "Kostenlos Preisüberwachen",
    },

    // Hero
    hero: {
      badge: "Smarte Hotel-Preisüberwachung — 100% kostenlos",
      headline: "Dein Hotel wurde|günstiger.|Wir sagen's dir.",
      sub: "Einmal eintragen. SaveMyHoliday überwacht Hotelpreise auf 10+ Plattformen täglich und benachrichtigt dich, sobald der Preis fällt.",
      cta: "Kostenlos Preisüberwachen →",
      trust1: "✓ 100% kostenlos",
      trust2: "✓ Einrichtung in 60 Sekunden",
      tracked: "1.385+ Buchungen überwacht",
      imgAlt: "SaveMyHoliday Preisüberwachung Demo",
    },

    // Stats
    stats: {
      bookings: "Buchungen überwacht",
      avgSaving: "Ø Ersparnis/Buchung",
      totalSaved: "Gesamt gespart",
      rating: "Nutzerbewertung",
    },

    // Features
    features: {
      eyebrow: "SO FUNKTIONIERT'S",
      headline: "So sparst du beim|Reisen!",
      sub: "Preise nach der Buchung überwachen und die Ersparnis selbst einstreichen — automatisch und völlig kostenlos.",
      card1Title: "Echtzeit-Überwachung",
      card1Text: "Dein Zimmer wird täglich auf 10+ Plattformen geprüft — vollautomatisch, ohne dass du einen Finger rühren musst.",
      card2Title: "Sofortige Preisalarme",
      card2Text: "Fällt der Preis, bist du sofort dabei. Du erhältst eine E-Mail mit dem neuen Preis und einem direkten Link zur Umbuchung.",
      card3Title: "Garantierte Ersparnis",
      card3Text: "Altes Ticket stornieren, günstiger neu buchen — fertig. Wir matchen nur exakt dein Zimmer und deine Daten, damit du wirklich sparst.",
    },

    // Live Scanning
    live: {
      eyebrow: "LIVE SCANNING",
      scanning: "Scanne",
      scanned: "Hotels gescannt",
      headline: "Wir scannen 10+ Plattformen",
      headlineSub: "täglich automatisch.",
      body: "Unser System prüft täglich automatisch die Preise auf den großen Buchungsplattformen — und findet günstigere Preise für exakt dein Zimmer und deine Daten.",
      morePlatforms: "+6 weitere",
    },

    // Comparison
    comparison: {
      eyebrow: "DER VERGLEICH",
      headline: "Kein Cent zu viel.|Mehr für die Urlaubskasse.",
      sub: "Vergiss manuelle Preischecks — SaveMyHoliday überwacht jede Plattform für dich und benachrichtigt dich sofort, wenn die Preise fallen.",
      without: "Ohne SaveMyHoliday",
      withTitle: "Mit SaveMyHoliday",
      withoutList: [
        "Zu viel für dein Hotel zahlen",
        "Preise manuell prüfen — dauert Stunden",
        "Preissenkungen im Schlaf verpassen",
        "Geld verlieren bei Buchungen, die du günstiger neu buchen könntest",
        "Keine Ahnung ob woanders günstigere Preise existieren",
      ],
      withList: [
        "Sofort benachrichtigt wenn der Preis fällt",
        "Wir scannen automatisch 10+ Plattformen täglich",
        "Nie wieder einen günstigeren Preis verpassen",
        "Neu buchen und sparen — in wenigen Klicks",
        "Immer wissen, dass du den besten Preis hast",
      ],
    },

    // Testimonials
    testimonials: {
      eyebrow: "KUNDENSTIMMEN",
      headline: "Zufriedene Reisende|sind unsere beste Werbung",
      items: [
        { text: "Dank SaveMyHoliday habe ich €180 bei meinem Paris-Hotel gespart. Ich hatte vorgebucht und es fast vergessen — dann plötzlich ein Alarm. Unglaublich!", name: "Sarah M.", country: "Deutschland" },
        { text: "Das ist das klügste Reise-Tool, das ich je benutzt habe. In 2 Minuten eingerichtet und noch in der gleichen Woche einen günstigeren Preis gefunden.", name: "Thomas R.", country: "USA" },
        { text: "SaveMyHoliday hat mir €300 bei einem Dubai-Hotel gespart. Der Alarm kam um 6 Uhr morgens und ich habe vor der Arbeit neu gebucht. Genial für Vielreisende.", name: "Jana K.", country: "Österreich" },
        { text: "€250 bei einem Rom-Hotel gespart. Die Alarme sind präzise — exakt das gleiche Zimmer, viel günstiger.", name: "Marco B.", country: "Italien" },
        { text: "Bester Reise-Hack ever. Nie wieder manuell Preise prüfen. Einrichtung war super einfach.", name: "Lisa H.", country: "UK" },
        { text: "Für Geschäftsreisen genutzt und meinem Unternehmen über €800 in einem Monat gespart.", name: "Ahmed K.", country: "VAE" },
      ],
    },

    // FAQ
    faq: {
      eyebrow: "FAQ",
      headline: "Hast du|eine Frage?",
      sub: "Wenn du Fragen hast, beantworten wir sie alle.",
      founderText: "Ich bin Zünnun, aus Berlin — Gründer von SaveMyHoliday. Ich reise gerne, aber ich kann es nicht ausstehen, zu viel für Hotels zu zahlen. Jeder spart gerne Geld, oder? Also habe ich SaveMyHoliday gebaut, um 10+ Plattformen automatisch zu scannen, damit Reisende wie du immer den besten Preis bekommen.",
      contactLabel: "Schreib mir:",
      contactEmail: "hello@savemyholiday.com",
      seeMore: "Vielleicht wurde deine Frage bereits beantwortet, schau mal hier.",
      items: [
        {
          q: "Was ist SaveMyHoliday?",
          a: "SaveMyHoliday ist ein kostenloser Service, der deinen Hotelbuchungspreis überwacht, nachdem du gebucht hast. Wenn der Preis fällt, benachrichtigen wir dich sofort per E-Mail, damit du günstiger neu buchen kannst.",
        },
        {
          q: "Wie verdient SaveMyHoliday Geld?",
          a: "Wir verdienen eine kleine Affiliate-Provision, wenn du über unsere Links neu buchst — ohne Mehrkosten für dich. Der Service ist für Nutzer vollständig kostenlos.",
        },
        {
          q: "Welche Plattformen scannt ihr?",
          a: "Wir scannen 10+ Plattformen, darunter Booking.com, Hotels.com, Expedia, Agoda und mehr. Wir gleichen dein genaues Hotel, Zimmertyp und Daten ab, damit du immer den besten verfügbaren Preis bekommst.",
        },
        {
          q: "Wie schnell werde ich benachrichtigt?",
          a: "Das hängt vom Hotel und Markt ab — manchmal fällt ein Preis innerhalb weniger Tage, manchmal zeigt er sich 2–4 Wochen vor Reisebeginn. Unser System scannt täglich, damit du keine Preissenkung verpasst.",
        },
        {
          q: "Wann höre ich von SaveMyHoliday?",
          a: "Wir melden uns nur, wenn wir einen günstigeren Preis für deine genaue Buchung finden. Du bekommst keine zufälligen E-Mails — nur echte Spar-Alarme.",
        },
        {
          q: "Sind meine Daten sicher?",
          a: "Absolut. Wir teilen deine Daten nie und nutzen deine E-Mail nur für Preissenkungsalarme. Du kannst deine Buchung jederzeit löschen.",
        },
      ],
    },

    // Global Coverage / Form
    form: {
      eyebrow: "WELTWEIT VERFÜGBAR",
      headline: "Hotels überall tracken,",
      headlineSub: "jederzeit.",
      formEyebrow: "JETZT STARTEN",
      formHeadline: "Deine Buchung eingeben,",
      formHeadlineSub: "wir erledigen den Rest.",
      formSub: "Dauert 60 Sekunden. Wir überwachen deinen Hotelpreis täglich automatisch.",
      success: "✅ Preisüberwachung gestartet! Du bekommst eine E-Mail sobald wir einen günstigeren Preis finden.",
      hotelName: "Hotelname",
      hotelPlaceholder: "z.B. Marriott Berlin Mitte",
      city: "Stadt",
      cityPlaceholder: "z.B. Berlin",
      country: "Land",
      countryPlaceholder: "z.B. Deutschland",
      roomType: "Zimmertyp",
      roomTypeNote: "(nur auf Englisch)",
      roomTypePlaceholder: "z.B. Deluxe Double Room",
      rooms: "Zimmer",
      adults: "Erwachsene",
      children: "Kinder",
      mealPlan: "Verpflegung",
      mealPlanPlaceholder: "Verpflegung wählen…",
      mealOptions: {
        roomOnly: "Nur Zimmer",
        breakfast: "Mit Frühstück",
        halfBoard: "Halbpension",
        fullBoard: "Vollpension",
        allInclusive: "All Inclusive",
      },
      checkin: "Check-in",
      checkout: "Check-out",
      price: "Gezahlter Preis",
      pricePlaceholder: "z.B. 289",
      currency: "Währung",
      email: "Deine E-Mail",
      emailPlaceholder: "du@email.de",
      saving: "Speichern...",
      cta: "Kostenlos Preisüberwachen →",
      tracked: "1.385+ Buchungen überwacht",
      ribbon: "100% Kostenlos",
      errorHotel: "Bitte gib den Hotelnamen ein.",
      errorCity: "Bitte gib die Stadt ein.",
      errorCheckin: "Bitte gib das Check-in Datum ein.",
      errorCheckout: "Bitte gib das Check-out Datum ein.",
      errorDates: "Check-out muss nach Check-in liegen.",
      errorEmail: "Bitte gib eine gültige E-Mail-Adresse ein.",
    },

    seoHome: {
      eyebrow: "MEHR ENTDECKEN",
      headline: "Die besten Seiten, um|schneller günstiger zu buchen",
      sub: "Wenn du verstehen willst, wann Hotelpreise fallen, welche Städte sich besonders lohnen und wie du nach der Buchung noch sparen kannst, findest du hier unsere stärksten Guides.",
      card1Title: "Hotel Price Tracker Guide",
      card1Text: "Verstehe genau, wie Preisüberwachung nach der Buchung funktioniert und wann sie sich wirklich lohnt.",
      card1Cta: "Zum Preis-Guide",
      card2Title: "Preisalarm nach Buchung",
      card2Text: "Die wichtigste Seite für alle, die nach der Hotelbuchung noch einen besseren Preis erwischen wollen.",
      card2Cta: "Zum Preisalarm-Guide",
      card3Title: "Beliebte Hotel-Städte",
      card3Text: "Entdecke starke Städte-Landingpages mit lokalen Spartipps, Saisonalität und passenden Hotel-Guides.",
      card3Cta: "Zu den Städte-Seiten",
      card4Title: "Neueste Guides im Blog",
      card4Text: "Praxisnahe Artikel rund um Preisstürze, kostenlose Stornierung und clevere Hotelbuchungen.",
      card4Cta: "Zum Blog",
    },

    // Footer
    footer: {
      tagline: "Buche dein Hotel. Wir beobachten den Preis. Wenn er fällt, benachrichtigen wir dich sofort — damit du stornieren und günstiger neu buchen kannst.",
      guides: "Guides",
      hotelPriceTracker: "Hotel Price Tracker",
      priceAlertAfterBooking: "Preisalarm nach Buchung",
      cityPages: "Städte-Guides",
      company: "Unternehmen",
      aboutUs: "Über uns",
      features: "Funktionen",
      pricing: "Preise",
      news: "Neuigkeiten",
      faq: "FAQ",
      resources: "Ressourcen",
      events: "Events",
      blog: "Blog",
      contactUs: "Kontakt",
      feedback: "Feedback",
      support: "Support",
      account: "Konto",
      supportCenter: "Hilfecenter",
      accessibility: "Barrierefreiheit",
      rights: "© 2026 SaveMyHoliday. Alle Rechte vorbehalten.",
      privacy: "Datenschutz",
      terms: "AGB",
      cookies: "Cookies",
      imprint: "Impressum",
    },

    // Emails
    emails: {
      alertSubject: "💰 Günstigerer Preis gefunden für {hotel} — SaveMyHoliday",
      alertTitle: "Günstigerer Preis gefunden!",
      alertBody: "Wir haben einen niedrigeren Preis für deine Buchung bei {hotel} gefunden. Du kannst jetzt {savings} sparen.",
      alertComparison: "Preisvergleich",
      alertCheckin: "Check-in",
      alertCheckout: "Check-out",
      alertYourPrice: "Dein Preis",
      alertNewPrice: "Neuer Preis bei",
      alertSaving: "Du sparst",
      alertCta: "Jetzt für {price} {currency} buchen →",
      alertTip: "Storniere zuerst deine aktuelle Buchung — stelle sicher, dass kostenlose Stornierung möglich ist.",
    },
  },

  en: {
    brand: "SaveMyHoliday",
    brandTagline: "SaveMyHoliday.",
    brandPart1: "Save",
    brandPart2: "My",
    brandPart3: "Holiday",

    nav: {
      home: "Home",
      howItWorks: "How it works",
      trackHotel: "Track Hotel",
      priceGuide: "Price Guide",
      cities: "Cities",
      faq: "FAQ",
      contact: "Contact",
      blog: "Blog",
      cta: "Start Tracking — Free",
      ctaMobile: "Start Tracking",
    },

    hero: {
      badge: "Smart Hotel Price Tracking — 100% Free",
      headline: "Your Hotel Got|Cheaper.|We'll Tell You.",
      sub: "Add your booking once. SaveMyHoliday monitors hotel prices across 10+ platforms daily and alerts you when the price drops.",
      cta: "Start Tracking — Free →",
      trust1: "✓ 100% Free",
      trust2: "✓ Setup in 60 seconds",
      tracked: "1,385+ Bookings Tracked",
      imgAlt: "SaveMyHoliday price tracking demo",
    },

    stats: {
      bookings: "Bookings Tracked",
      avgSaving: "Avg. Saving/Booking",
      totalSaved: "Total Saved",
      rating: "User Rating",
    },

    features: {
      eyebrow: "HOW IT WORKS",
      headline: "That's The Way|To Save!",
      sub: "Track prices after booking and keep the savings for yourself — automatically and completely free.",
      card1Title: "Real-Time Monitoring",
      card1Text: "We scan 10+ platforms daily — for your exact room and dates, fully automatically.",
      card2Title: "Instant Price Alerts",
      card2Text: "The moment a cheaper price appears, you get an email with the new rate and a direct rebook link.",
      card3Title: "Guaranteed Savings",
      card3Text: "Cancel and rebook at the lower price. Smart matching ensures we only alert you for the exact same room.",
    },

    live: {
      eyebrow: "LIVE SCANNING",
      scanning: "Scanning",
      scanned: "hotels scanned",
      headline: "We Scan 10+ Platforms",
      headlineSub: "Every Single Day.",
      body: "Every day, our system automatically checks prices across the major booking platforms — finding you cheaper rates for the exact same room and dates.",
      morePlatforms: "+6 more",
    },

    comparison: {
      eyebrow: "THE COMPARISON",
      headline: "Stop Overpaying.|Start Saving.",
      sub: "Forget checking prices manually — SaveMyHoliday watches every platform for you and alerts you the instant rates drop.",
      without: "Without SaveMyHoliday",
      withTitle: "With SaveMyHoliday",
      withoutList: [
        "Overpay for your hotel",
        "Check prices manually — takes hours",
        "Miss price drops while you sleep",
        "Lose money on bookings you could rebook",
        "No idea if cheaper rates exist elsewhere",
      ],
      withList: [
        "Get alerted the moment the price drops",
        "We scan 10+ platforms automatically every day",
        "Never miss a cheaper rate again",
        "Rebook and save — in just a few clicks",
        "Always know you got the best price",
      ],
    },

    testimonials: {
      eyebrow: "TESTIMONIALS",
      headline: "Happy Travelers Are|Our Best Ads",
      items: [
        { text: "Thanks to SaveMyHoliday I saved €180 on my Paris hotel. I booked in advance and forgot about it — then suddenly got an alert. Incredible!", name: "Sarah M.", country: "Germany" },
        { text: "I think this is the smartest travel tool I've ever used. Set it up in 2 minutes and it found me a cheaper rate the same week.", name: "Thomas R.", country: "USA" },
        { text: "SaveMyHoliday saved me €300 on a Dubai hotel. The alert came at 6am and I rebooked before work. Life changing for frequent travelers.", name: "Jana K.", country: "Austria" },
        { text: "Saved €250 on a Rome hotel. The alerts are spot on — exact same room, much cheaper.", name: "Marco B.", country: "Italy" },
        { text: "Best travel hack ever. Never checking prices manually again. Setup was super easy.", name: "Lisa H.", country: "UK" },
        { text: "Used it for business travel and saved my company over €800 in one month.", name: "Ahmed K.", country: "UAE" },
      ],
    },

    faq: {
      eyebrow: "FAQ",
      headline: "Got A |Question?",
      sub: "If there are questions you want to ask, we will answer all your questions.",
      founderText: "I'm Zünnun, based in Berlin — solo founder of SaveMyHoliday. I love to travel but I can't stand overpaying for hotels. Everyone likes to save money, right? So I built SaveMyHoliday to scan 10+ platforms automatically so travelers like you always get the best rate.",
      contactLabel: "Write to me:",
      contactEmail: "hello@savemyholiday.com",
      seeMore: "Maybe your question has been answered, check this out.",
      items: [
        { q: "What is SaveMyHoliday?", a: "SaveMyHoliday is a free service that monitors your hotel booking price after you've booked. When the price drops, we instantly alert you by email so you can rebook cheaper." },
        { q: "How does SaveMyHoliday make money?", a: "We earn a small affiliate commission when you rebook through our links — at no extra cost to you. The service is completely free for users." },
        { q: "Which platforms do you scan?", a: "We scan 10+ platforms including Booking.com, Hotels.com, Expedia, Agoda, and more. We match your exact hotel, room type, and dates to make sure you always get the best available rate." },
        { q: "How quickly do I get alerted?", a: "It depends on the hotel and market — sometimes a price drop happens within a few days, other times it shows up 2–4 weeks before your trip begins. Our system scans daily so you never miss a significant drop." },
        { q: "When will I hear from SaveMyHoliday?", a: "We only contact you when we find a cheaper rate for your exact booking. You won't receive random emails — only real savings alerts." },
        { q: "Is my data safe?", a: "Absolutely. We never share your data and only use your email to send price drop alerts. You can delete your booking anytime." },
      ],
    },

    form: {
      eyebrow: "GLOBAL COVERAGE",
      headline: "Track Hotels Anywhere,",
      headlineSub: "Anytime.",
      formEyebrow: "START TRACKING",
      formHeadline: "Enter Your Booking,",
      formHeadlineSub: "We Do The Rest.",
      formSub: "Takes 60 seconds. We monitor your hotel price automatically every day.",
      success: "✅ Price tracking started! We'll email you as soon as we find a cheaper price.",
      hotelName: "Hotel Name",
      hotelPlaceholder: "e.g. Marriott Berlin Mitte",
      city: "City",
      cityPlaceholder: "e.g. Berlin",
      country: "Country",
      countryPlaceholder: "e.g. Germany",
      roomType: "Room Type",
      roomTypeNote: "(English only)",
      roomTypePlaceholder: "e.g. Deluxe Double Room",
      rooms: "Rooms",
      adults: "Adults",
      children: "Children",
      mealPlan: "Meal Plan",
      mealPlanPlaceholder: "Select meal plan…",
      mealOptions: {
        roomOnly: "Room Only",
        breakfast: "Breakfast Included",
        halfBoard: "Half Board",
        fullBoard: "Full Board",
        allInclusive: "All Inclusive",
      },
      checkin: "Check-in",
      checkout: "Check-out",
      price: "Price You Paid",
      pricePlaceholder: "e.g. 289",
      currency: "Currency",
      email: "Your Email",
      emailPlaceholder: "you@email.com",
      saving: "Saving...",
      cta: "Start Tracking — Free →",
      tracked: "1,385+ Bookings Tracked",
      ribbon: "100% Free",
      errorHotel: "Please enter the hotel name.",
      errorCity: "Please enter the city.",
      errorCheckin: "Please enter your check-in date.",
      errorCheckout: "Please enter your check-out date.",
      errorDates: "Check-out must be after check-in.",
      errorEmail: "Please enter a valid email address.",
    },

    seoHome: {
      eyebrow: "EXPLORE MORE",
      headline: "The Best Pages To|Book Smarter",
      sub: "If you want to understand when hotel prices drop, which cities are worth tracking, and how to save after booking, these are the strongest pages to start with.",
      card1Title: "Hotel Price Tracker Guide",
      card1Text: "Understand how post-booking hotel price tracking works and when it creates real savings.",
      card1Cta: "See the Guide",
      card2Title: "Price Alert After Booking",
      card2Text: "Our most important page for travelers who want a cheaper hotel rate after they already booked.",
      card2Cta: "See the Alert Guide",
      card3Title: "Popular Hotel Cities",
      card3Text: "Discover city landing pages with local saving context, seasonality and useful hotel insights.",
      card3Cta: "Browse Cities",
      card4Title: "Latest Blog Guides",
      card4Text: "Practical articles about hotel price drops, free cancellation and smarter travel booking decisions.",
      card4Cta: "Open the Blog",
    },

    footer: {
      tagline: "Book your hotel. We watch the price. If it drops, we alert you instantly — so you can cancel and rebook for less.",
      guides: "Guides",
      hotelPriceTracker: "Hotel Price Tracker",
      priceAlertAfterBooking: "Price Alert After Booking",
      cityPages: "City Guides",
      company: "Company",
      aboutUs: "About Us",
      features: "Features",
      pricing: "Pricing",
      news: "News",
      faq: "FAQ",
      resources: "Resources",
      events: "Events",
      blog: "Blog",
      contactUs: "Contact Us",
      feedback: "Feedback",
      support: "Support",
      account: "Account",
      supportCenter: "Support Center",
      accessibility: "Accessibility",
      rights: "© 2026 SaveMyHoliday. All rights reserved.",
      privacy: "Privacy Policy",
      terms: "Terms",
      cookies: "Cookies",
      imprint: "Legal Notice",
    },

    emails: {
      alertSubject: "💰 Cheaper price found for {hotel} — SaveMyHoliday",
      alertTitle: "Cheaper price found!",
      alertBody: "We found a lower price for your booking at {hotel}. You can save {savings} right now.",
      alertComparison: "Price Comparison",
      alertCheckin: "Check-in",
      alertCheckout: "Check-out",
      alertYourPrice: "Your price",
      alertNewPrice: "New price on",
      alertSaving: "You save",
      alertCta: "Book Now at {price} {currency} →",
      alertTip: "Cancel your current booking first — make sure it's free cancellation.",
    },
  },
} as const;

export type Translations = typeof translations.de;

export function getTranslations(locale: Locale): Translations {
  return translations[locale] as unknown as Translations;
}
